...
<div class="container">
    ...
    <div class="m-4 rounded-lg border border-gray-100 p-4 bg-gray-400 shadow-2xl">
        ...
        <div class="flex">
            ...
            <p class="font-thin fs-5 italic">{{ $article->created_at }}</p>
            <p class="ml-10 font-thin fs-5">本文共獲得</p><p class="ml-1 mr-1 font-bold fs-5">{{ $collection->count() }}</p><p class="font-thin fs-5">個回覆</p>
        </div>
        <!-- 作者登入才看到編輯刪除按鈕 -->
        @if (Route::has('login'))
            @auth
